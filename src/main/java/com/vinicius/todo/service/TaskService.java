...
public class TaskService {
    private TaskRepository taskRepository; // objeto que é a interface que contém os métodos a serem implementados

    public TaskService(TaskRepository taskRepository) {
        this.taskRepository = taskRepository; // aqui o spring vai escrever o conteúdo dos métodos da interface de TaskRepository
    }
    ...
    public Task uptade(Task task) throws Exception { // Não existe um metodo específico para dar update
        ...
        if (op.isPresent()) { // se for achado
            Task tUpdate = op.get(); // tUpdate = conteúdo da tarefa antiga
